...
export const handleConfirmRemove = (
  ...
  collectionList: Array<CollectionList>[],
  removeAnimeFromCollection: any,
  animeList: AnimeCollectionList
) => {
  const newArrayExceptId = collectionList.filter((x: any) => x.id !== state.selectedCollection.id)
  const removeAnimeByDeletedCollectionId = animeList.filter((x: any) => x.collectionId !== state.selectedCollection.id)
  updateCollections(newArrayExceptId)
  removeAnimeFromCollection(removeAnimeByDeletedCollectionId)
